# CONSTANTES

ANCHO, ALTO = 940, 940
FPS = 60
TITULO = 'Project Get that cheese'
BACK_COLOR = (18, 180, 95)
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)

points= 0
lives= 3


#spawneo

CHEESE_SPAWN_RATE = 180
ENEMY_SPAWN_RATE = 20  # Cada 1 segundo a 60 FPS
ITEM_SPAWN_RATE = 240
has_cheese = False

#sprites
PLAYER_IMG = 'src/rat_sprite-removebg-preview.png' 
ENEMY_IMG = 'src/enemyratgame.png'
WIN_IMG = 'src/Victory.png'
LOSE_IMG= 'src/Loser rat.png'
BG_IMG = 'src/Woodfloocheeseproye.png'
TEXT_FONT = 'src/Honk-Regular-VariableFont_MORF,SHLN.ttf'
CHEESE_IMG= 'src/Cheeseratgame-removebg-preview.png'
ITEM_IMG= 'src/pillitem-removebg-previewlk.png'
